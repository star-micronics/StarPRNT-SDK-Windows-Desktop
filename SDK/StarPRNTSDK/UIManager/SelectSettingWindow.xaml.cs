    ...
    public partial class SelectSettingWindow : Window
    {
        ...
        private void SetTemplete()
        {
            switch (TempleteType)
            {
                ...
                case Templete.LabelType:
                    SettingTitle = "Select label type.";
                    LabelTypeCollection = (LabelType[])Enum.GetValues(typeof(LabelType));
        ...
        public LabelType[] LabelTypeCollection
        {
            get
            {
                return labelTypeCollection;
            }
            set
            {
                labelTypeCollection = new LabelType[value.Length];

                Array.Copy(value, 0, labelTypeCollection, 0, labelTypeCollection.Length);
        ...
        private LabelType[] labelTypeCollection;

        public LabelType SelectedLabelType
        {
            get
            {
                return labelTypeCollection[SelectedIndex];
            }
        }
        ...
        private void LabelTypeToString()
        {
            settingsArray = new string[labelTypeCollection.Length];

            for (int i = 0; i < settingsArray.Length; i++)
            {
                settingsArray[i] = GetLabelTypeDescription(labelTypeCollection[i]);
            }
        }

        private string GetLabelTypeDescription(LabelType type)
        {
            string description = "";
            ...
            switch (type)
            {
                ...
                case LabelType.Valid:
                    description = "Valid";
                    break;

                case LabelType.ValidWithDetection:
                    description = "Valid With Detection";
                    break;
            }
